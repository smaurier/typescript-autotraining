...
- Créer des types utilitaires avances : Flatten, UnpackPromise, IsEqual
- Comprendre la distribution des types conditionnels sur les unions
- Écrire des assertions de type au niveau compilation (type-level tests)
- Maîtriser IsNever et les cas limites des conditional types
...
3. **Types conditionnels pratiques** — Implementer `ReturnTypeOf<T>`, `ParametersOf<T>`, `PromisifyAll<T>` pour des cas d'usage réels.
