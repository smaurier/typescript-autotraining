...
1. **`Length<T>`** -- Calculer la longueur d'un type tuple
2. **`BuildTuple<N>`** -- Construire un tuple de taille N
3. **`Add<A, B>`** -- Additionner deux nombres au niveau des types
...
```bash
npx ts-node lab-13-type-programming/exercise.ts
npx ts-node lab-13-type-programming/solution.ts
```
